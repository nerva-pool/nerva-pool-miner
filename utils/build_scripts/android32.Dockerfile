...
RUN cd boost_${BOOST_VERSION} \
    && ./b2 --build-type=minimal link=static runtime-link=static --with-chrono --with-date_time --with-filesystem --with-program_options --with-regex --with-serialization --with-system --with-thread --build-dir=android32 --stagedir=android32 toolset=clang threading=multi threadapi=pthread target-os=android -sICONV_PATH=${WORKDIR}/libiconv  stage -j${NPROC}
...
RUN cd /src && \
    ...
    cmake ../../../translations && make -j${NPROC}
...
RUN cd /src/build/release \
    && BOOST_ROOT=${WORKDIR}/boost_${BOOST_VERSION} BOOST_LIBRARYDIR=${WORKDIR}/boost_${BOOST_VERSION}/android32/lib/ \
     ...
     CXX=arm-linux-androideabi-clang++ cmake \
     -D BUILD_TESTS=OFF \
     -D ARCH="armv7-a" -D STATIC=ON -D BUILD_64=OFF \
     -D CMAKE_BUILD_TYPE=release \
     -D ANDROID=true -D INSTALL_VENDORED_LIBUNBOUND=ON -D BUILD_TAG="android" \
     -D CMAKE_SYSTEM_NAME="Android" \
     -D CMAKE_ANDROID_STANDALONE_TOOLCHAIN="${ANDROID_STANDALONE_TOOLCHAIN_PATH}" \
     -D CMAKE_ANDROID_ARM_MODE=ON \
     -D CMAKE_ANDROID_ARCH_ABI="armeabi-v7a" ../.. && make -j${NPROC}
